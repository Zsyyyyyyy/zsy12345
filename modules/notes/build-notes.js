/**
 * 把 data/ 目录下的 .md 文件打包成静态数据文件 notes-data.js
...
const NOTES_DIR = path.join(ROOT, 'data');
const OUT_FILE = path.join(ROOT, 'notes-data.js');

function addNoteFile(p, region, out) {
  const f = path.basename(p);
  const stat = fs.statSync(p);
  const lower = f.toLowerCase();
  const rel = path.relative(NOTES_DIR, p).replace(/\\/g, '/');
  ...
  if (lower.endsWith('.md')) {
    ...
    out.push({
      file: rel,
      title: f.replace(/\.(xlsx|xls|csv)$/i, ''),
      region: region,
      type: 'xlsx',
      created: Math.round((stat.birthtimeMs || stat.mtimeMs)),
      updated: Math.round(stat.mtimeMs),
...
function walkNotes(dir, region, out) {
  if (!fs.existsSync(dir)) return;
  fs.readdirSync(dir).forEach(f => {
    const p = path.join(dir, f);
    const stat = fs.statSync(p);
    if (stat.isDirectory()) {
      walkNotes(p, region, out);
    } else {
      addNoteFile(p, region, out);
    }
  });
}
...
function listNotes() {
  ...
  // 根目录下的文件归「其他」
  fs.readdirSync(NOTES_DIR).forEach(f => {
    const p = path.join(NOTES_DIR, f);
    if (!fs.statSync(p).isDirectory()) {
      addNoteFile(p, '其他', out);
    }
  });

  // 各子目录递归扫描，region = 目录名
  fs.readdirSync(NOTES_DIR).forEach(sub => {
    const subPath = path.join(NOTES_DIR, sub);
    if (fs.statSync(subPath).isDirectory()) {
      walkNotes(subPath, sub, out);
    }
  });
